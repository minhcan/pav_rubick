...
class ControllerExtensionModulepavhomebuilder extends Controller {
	...
	public function _breadcrumbs(){
		...
		'href'      => $this->url->link('extension/module/pavcustom', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	}
	...
	public function _getStores(){
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('extension/module/pavcustom', $url.'&token=' . $this->session->data['token'], 'SSL');
		}
		return $stores;
	}
